  ...
  if(dialogid == 100) {
    if(response) {
      switch (listitem) {
        ...
        case 2: {
          StopAudioStreamForPlayer(playerid);
          SendClientMessage(playerid, COLOR_RED, "Vypnuli jste si rádio. Pro zapnutí rádia napiąte /radio");
        }
        case 3: {
          ShowPlayerDialog(playerid,103,DIALOG_STYLE_MSGBOX,"{FF3333}<<<Radio>>>"," * Radio FS\n * © by Roman 'Roman3349' Ondráček\n * Skype: ondracek.roman ","Konec","");
        }
      }
    }
  }
    ...
    if(response) {
      switch (listitem) {
       case 0: {
         ...
         SendClientMessage(playerid, COLOR_RED, "Pro vypnutí rádia napiąte /stop nebo /radio");
       }
       case 1: {
         StopAudioStreamForPlayer(playerid);
